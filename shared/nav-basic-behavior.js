...
// and after shared/auth-cookie-client.js (for window.CarryParcelAuth).
// =========================================================
(function () {
    'use strict';
    ...
    window.CPNav = {
        initialized: false,
        init: function() {
            if (window.CPNav.initialized) return;
            ...
            if (menuToggle && mainNav) {
                ...
                mainNav.querySelectorAll('a, button').forEach(function (link) {
                    ...
                    link.addEventListener('click', function(e) {
                        ...
                        // For internal anchor links (hashes), close the menu so the user sees the content.
                        // For external page links, do NOT close the menu here - let the browser start
                        // navigation. Closing it immediately can cancel the navigation on some mobile browsers.
            ...
            // ---- Shadow on scroll ----
            var navHeaderEl = document.querySelector('.cp-nav-header');
            ...
            if (guestActions && chip) {
                function readUser() {
                    try { return JSON.parse(localStorage.getItem('carryParcelUser') || '{}'); }
                    catch (e) { return {}; }
                }
                function readAdmin() {
                    try { return JSON.parse(localStorage.getItem('carryParcelAdmin') || '{}'); }
                    catch (e) { return {}; }
                }
                function fullName(user) {
                    return (((user.firstName || '') + ' ' + (user.lastName || '')).trim()) || user.name || 'CarryParcel';
                }
                function initials(name) {
                    return name.split(/\s+/).filter(Boolean).slice(0, 2).map(function (p) { return p[0].toUpperCase(); }).join('') || 'CP';
                }
                function renderNavbarAvatar(avatarEl, user, name) {
                    if (!avatarEl) return;
                    var photo = user?.profilePhoto || '';
                    avatarEl.style.backgroundImage = '';
                    avatarEl.querySelectorAll('img.cp-profile-photo').forEach(function (img) { img.remove(); });
                    if (photo) {
                ...
                var isAdmin = Boolean(localStorage.getItem('carryParcelAdmin'));
                var isUser = Boolean(localStorage.getItem('carryParcelUser'));
                ...
                document.removeEventListener('click', window._cpNavClickHandler);
                window._cpNavClickHandler = function () {
                    chip.classList.remove('open');
                    trigger && trigger.setAttribute('aria-expanded', 'false');
                };
                document.addEventListener('click', window._cpNavClickHandler);
                ...
                if (logoutBtn) {
                    logoutBtn.addEventListener('click', function () {
                        if (localStorage.getItem('carryParcelAdmin')) {
                            if (window.CarryParcelAuth && typeof window.CarryParcelAuth.logoutAdmin === 'function') {
                                window.CarryParcelAuth.logoutAdmin().finally(function () { window.location.href = '/'; });
                            } else {
    ...
    if (document.readyState === 'complete' || document.readyState === 'interactive') {
        ...
        document.addEventListener('DOMContentLoaded', window.CPNav.init);
    }
})();
