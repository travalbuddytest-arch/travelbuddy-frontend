...
function build(){
  ...
  <button class="tb-banner-reopen" id="tbBannerReopen" type="button">Manage Cookies</button>`);
}
function init(){
  build();
  ...
  function showBanner(){banner.classList.add('show');backdrop.classList.add('show','tb-backdrop-behind-banner');}
  function hideBanner(){banner.classList.remove('show');backdrop.classList.remove('show','tb-backdrop-behind-banner');}
  function finish(data){write(data);hideBanner();closeModal();settings.classList.add('show');}
...
window.TravelBuddyCookies={get:()=>read()||{...defaults},has:type=>type==='necessary'||!!(read()||defaults)[type],open:()=>document.getElementById('tbBannerReopen')?.click()};
